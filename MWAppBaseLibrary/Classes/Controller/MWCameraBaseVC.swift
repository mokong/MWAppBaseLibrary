...
//  MWAppBase
//
//  Created by MorganWang on 11/1/2022.
//
...
class MWCameraBaseVC: UIViewController {
    ...
    var singleTapCount: Int64 = 0 {
        didSet {
            handleTapCountChange()
        }
    }
...
//        }
    ...
    fileprivate func observeDeviceMotion() {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(handleAppBecomeActive(_:)),
                                               name: UIApplication.didBecomeActiveNotification,
                                               object: nil)
        
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(handleAppEnterBack(_:)),
                                               name: UIApplication.didEnterBackgroundNotification,
                                               object: nil)
    }
    
    @objc fileprivate func handleAppBecomeActive(_ sender: Notification) {
        if self.view.window != nil {
            self.session?.startRunning()
        }
    }
    
    @objc fileprivate func handleAppEnterBack(_ sender: Notification) {
        if session?.isRunning == true {
            session?.stopRunning()
        }
    }
