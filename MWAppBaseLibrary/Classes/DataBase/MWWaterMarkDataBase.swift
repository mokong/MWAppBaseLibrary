//
//  MWWaterMarkDataBase.swift
//  MWWaterMarkCamera
//
//  Created by Horizon on 9/3/2022.
//
...
class MWWaterMarkDataBase {
    ...
    // MARK: - init
    static let shardDB = MWWaterMarkDataBase()
    fileprivate init() {
        databasePath = MWWaterMarkDataBase.getDBFilePath()
//        reInitDB()
...
//        }
    ...
    static func getDBFilePath() -> String {
        if let path = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).first {
            let dbPath = (path as NSString).appendingPathComponent("game24.db")
            print("数据库路径：%@", dbPath)
            return dbPath
        }
        return ""
